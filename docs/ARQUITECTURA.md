...
├── contenedor.py          raíz de composición (arma las dependencias)
├── db_config.ini          credenciales de BD (NO se versiona)
├── db_config.example.ini  plantilla de configuración
├── datos/                 capa de datos (repositorios + conexión)
│   ├── db.py  repositorio.py  repositorio_base.py  interfaces.py
│   └── repo_usuarios.py  repo_jornadas.py  repo_produccion.py
│       repo_fabricacion.py  repo_catalogos.py  repo_indicadores.py
├── servicios/             lógica de negocio
│   └── auth.py  servicio_auth.py  servicio_proyeccion.py
│       almacenamiento_sesion.py  validaciones.py
├── interfaz/              presentación (pantallas + soporte de UI)
│   ├── login_ui.py  inicio_ui.py  recolec_ui.py  transformacion_ui.py
│   │   impresion_ui.py  historico_ui.py  admin_ui.py
│   ├── widgets.py  ui_tema.py  ui_imagenes.py  ui_ventana.py
│   │   ui_dialogos.py  ui_navbar.py  ui_utils.py
│   └── soporte_dialog.py  privacidad.py  use_terms.py
├── pruebas/               pruebas_humo.py  pruebas_servicios.py
├── scripts/               seed_datos.py  seed_usuario.py
├── recursos/logos/        imágenes (íconos y logo)
├── docs/                  ARQUITECTURA.md
├── DB/                    respaldo de la base (vortrack_db.bacpac)
└── maquetas_html/         mockups HTML de cada pantalla
```

`datos/`, `servicios/` e `interfaz/` son **paquetes** de Python (tienen
`__init__.py`), así que los módulos se importan cualificados por paquete
(`import datos.repositorio as repositorio`, `from interfaz.ui_tema import COLORS`).
Todo se ejecuta desde la raíz con `python app.py` (o `python pruebas/…`).
