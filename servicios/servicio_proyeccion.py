...
Unidades: modelos en minutos y gramos; filamento producido en kg.
"""


class ServicioProyeccion:
    @staticmethod
    def proyeccion_impresion(tiempo_estimado_min, peso_estimado_g, cantidad):
        """Devuelve (peso_total_g, tiempo_total_min). El tiempo estimado del
        modelo ya está en minutos, así que solo se multiplica por la cantidad.
        Cada valor es None si falta el estimado correspondiente en el modelo."""
        try:
        ...
        tiempo_total = (int(round(float(tiempo_estimado_min) * cantidad))
                        if tiempo_estimado_min is not None else None)
        return peso_total, tiempo_total

    @staticmethod
    def piezas_por_filamento(peso_kg, modelos):
        """`modelos`: iterable de (id, nombre, categoria, tiempo_min, peso_g).

        Devuelve (filas, mejor, disponible_g) donde:
          - filas: [(nombre, piezas|None, g_pieza|None, sobrante_g|None, tiempo_total_min|None), ...]
        ...
        filas = []
        mejor = None
        for _id, nombre, _cat, tiempo_min, peso_g in modelos:
            ...
            tiempo_total = float(tiempo_min) * piezas if tiempo_min else None
